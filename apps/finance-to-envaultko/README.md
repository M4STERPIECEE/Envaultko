# finance-to-envaultko

Idempotent one-shot migration of legacy **finance** data into **envaultko**.
...
Maps finance users, tags, and operations into envaultko users, tags,
...
allocations are given fresh cuid2 IDs (envaultko's `Id` value object rejects
...
root so the pnpm workspace lockfile is available to `pnpm install`):
    ...
    docker build -f apps/finance-to-envaultko/Dockerfile -t finance-to-envaultko .

Run it against the two databases:

    docker run --rm finance-to-envaultko \
      --finance=postgres://user:pass@host:5432/finance \
      --envaultko=postgres://user:pass@host:5432/envaultko

## Run locally

    pnpm --filter finance-to-envaultko start -- \
      --finance=postgres://... --envaultko=postgres://...

## Test

End-to-end verification against ephemeral Postgres containers (requires Docker):

    pnpm --filter finance-to-envaultko test
