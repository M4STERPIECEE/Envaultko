...
const ENVAULTKO_MIGRATIONS_DIR = fileURLToPath(
  new URL(
...
export async function applyEnvaultkoSchema(client: pg.Client): Promise<void> {
  const files = (await readdir(ENVAULTKO_MIGRATIONS_DIR))
    .filter((file) => file.endsWith(".sql"))
    .sort();
  for (const file of files) {
    const sql = await readFile(
      path.join(ENVAULTKO_MIGRATIONS_DIR, file),
      "utf8",
    );
