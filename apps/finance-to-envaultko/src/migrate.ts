...
// envaultko stores amounts as bigint but maps them to JS numbers, so the real
// ceiling is float64 integer precision rather than the column width.
const MAX_ENVAULTKO_AMOUNT_CENTS = Number.MAX_SAFE_INTEGER;
...
export async function migrate(opts: {
  ...
  const envaultko = new pg.Client({
    connectionString: opts.envaultkoUrl,
    connectionTimeoutMillis: 15_000,
  });
  await connectOrThrow(finance, "finance");
  await connectOrThrow(envaultko, "envaultko");
  ...
  try {
    ...
    for (const user of users) {
      ...
      const defaultPot = await envaultko.query<{ id: string }>(
    ...
    for (const op of operations) {
      ...
      if (amountCents > MAX_ENVAULTKO_AMOUNT_CENTS) {
        throw new Error(
          `Operation ${op.id} amount ${op.amount} exceeds envaultko's maximum representable amount`,
        );
      }
      if (amountCents <= 0) {
        console.warn(
    ...
    await envaultko.query("COMMIT");
    return summary;
  } catch (error) {
    await envaultko.query("ROLLBACK");
    throw error;
  } finally {
    await finance.end();
    await envaultko.end();
