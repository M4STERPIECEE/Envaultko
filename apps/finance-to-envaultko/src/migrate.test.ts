...
describe("migrate: users, pots, tags", () => {
  it("migrates every user", async () => {
    expect(await count(envaultko, '"user"')).toBe(seed.userIds.length);
  });
  ...
  it("falls back to email when the finance name is null", async () => {
    ...
    for (const row of nullNameUsers.rows) {
      const migrated = await envaultko.query<{ name: string }>(
  ...
  it("creates exactly one default pot per user at 100%", async () => {
    expect(await count(envaultko, "pots")).toBe(seed.userIds.length);
    const pots = await envaultko.query<{
      percentage: number;
      is_default: boolean;
  ...
  it("creates exactly one allocation per income/expense transaction", async () => {
    ...
    expect(await count(envaultko, "pot_allocations")).toBe(incomeOps.length);
    expect(await count(envaultko, "expense_allocations")).toBe(
      expenseOps.length,
    );
  });

  it("allocates the full transaction amount into the user's default pot", async () => {
    const rows = await envaultko.query<{
      tx_amount: number;
      alloc_amount: number;
      is_default: boolean;
    }>(
  ...
  it("assigns valid cuid ids to migrated pots and allocations", async () => {
    const ids = await envaultko.query<{ id: string }>(
      `SELECT id FROM pots
  ...
  it("migrates operation-tag links to transaction_tags", async () => {
    expect(await count(envaultko, "transaction_tags")).toBe(
  ...
  it("is idempotent for users, pots, and tags", async () => {
    const before = {
      users: await count(envaultko, '"user"'),
      pots: await count(envaultko, "pots"),
      tags: await count(envaultko, "tags"),
    };
    await migrate({ financeUrl, envaultkoUrl });
    expect(await count(envaultko, '"user"')).toBe(before.users);
    expect(await count(envaultko, "pots")).toBe(before.pots);
    expect(await count(envaultko, "tags")).toBe(before.tags);
  });
});
